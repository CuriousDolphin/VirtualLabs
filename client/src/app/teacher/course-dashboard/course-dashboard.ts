...
export class CourseDashboard implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.courseSubscription = this._currentCourse$.subscribe((course: Course) => {
      this.currentCourse = course;
    })

    this.routeSubscription = this.route.url.subscribe((evt) => {
      this.currentPath = evt[0].path, console.log('tab route change', 'data', this.currentPath);
  ...
  getCourse(name: string) {
    this.isLoading = true;
    this.courseSubscription = this.courseService.getCourse(name)
